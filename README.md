...
macOS 14 以降。トラックボールを制御するため、次の許可が必要です（初回起動時にガイドが出ます）。

| 権限 | 用途 | 必須 |
|---|---|---|
| アクセシビリティ | 左右クリックの監視、押しっぱなしジェスチャーの判定 | 必須 |
| 入力監視 | トラックボール本体との通信、ボタン入力の受け取り | 必須 |
| 操作送信 | 割り当てたキー操作・クリックの実行 | 付随 |

**「操作送信」にはシステム設定の項目がありません。** `CGPreflightPostEventAccess` が見ている TCC の `kTCCServicePostEvent` で、通常はアクセシビリティの許可に含まれます。ユーザーが直接オンにする手段がないため、この値が false でも他の機能は止めません。
...
- ポインタの移動を横取りするのは、スクロールモードと方向ジェスチャーのときだけです。アクションを割り当てただけのボタンでは、ドラッグも通常どおり使えます
